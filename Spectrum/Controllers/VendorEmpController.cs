...
[Authorize]
public class VendorEmpController : ControllerBase
{
    private readonly IVendorEmpService _service;

    public VendorEmpController(IVendorEmpService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateVendorEmpDTO dto)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateVendorEmpDTO dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
