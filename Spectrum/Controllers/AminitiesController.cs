...
{
    private readonly IAminitiesService _service;

    public AminitiesController(IAminitiesService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateAminitiesDTO dto)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateAminitiesDTO dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
