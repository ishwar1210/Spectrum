...
public class ApprovalStatusController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateApprovalStatusDTO dto)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateApprovalStatusDTO dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
