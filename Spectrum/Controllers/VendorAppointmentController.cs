...
[Authorize]
public class VendorAppointmentController : ControllerBase
{
    private readonly IVendorAppointmentService _service;

    public VendorAppointmentController(IVendorAppointmentService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateVendorAppointmentDTO dto)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateVendorAppointmentDTO dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
